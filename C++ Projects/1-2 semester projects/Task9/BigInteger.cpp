...
const long long NOTATION = 100;

template <typename T>
BigInteger<T>::BigInteger(T number)
{
...
template <typename T>
BigInteger<T>::BigInteger(const BigInteger<T>& bigInteger1)
{
...
template <typename T>
BigInteger<T>::BigInteger(BigInteger<T>&& bigInteger1)
{
...
template <typename T>
BigInteger<T>& BigInteger<T>::operator=(const BigInteger<T>& bigInteger1)
{
    if (this == &bigInteger1)
        return *this;
    _value = bigInteger1._value;
...
template <typename T>
BigInteger<T>& BigInteger<T>::operator=(BigInteger<T>&& bigInteger1)
{
    if (this == &bigInteger1)
        return *this;
    _value = std::move(bigInteger1._value);
...
template <typename T>
bool BigInteger<T>::operator== (const BigInteger<T>& bigInteger2) const
{
...
template <typename T>
BigInteger<T>& BigInteger<T>::summ(const BigInteger<T>& bigInteger2)
{
    int additive = 0;
    int i = 0;
    while (true)
...
template <typename T>
BigInteger<T>& BigInteger<T>::analizeSignForSumm(const BigInteger<T>& bigInteger2)
{
...
template <typename T>
BigInteger<T>& BigInteger<T>::operator+=(const BigInteger<T>& bigInteger2)
{
    if (*this >= bigInteger2)
...
template <typename T>
BigInteger<T> BigInteger<T>::operator+(const BigInteger<T>& bigInteger2) const
{
...
template <typename T>
BigInteger<T>& BigInteger<T>::difference(const BigInteger<T>& bigInteger2)
{
    int i = 0;
    int decreaser = 0;
...
template <typename T>
BigInteger<T>& BigInteger<T>::analizeSignForDifference(const BigInteger<T>& bigInteger2)
{
...
template <typename T>
std::pair<BigInteger<T>, BigInteger<T>> BigInteger<T>::division(const BigInteger<T>& bigInteger2)
{
    BigInteger resultBigInteger = BigInteger();
    BigInteger currentReducedBigInteger = BigInteger();
    bool isDividerANull = true;
    if (bigInteger2._value == BigInteger<T>(0)._value)
    ...
    while (true)
    {
        ...
        if (bigInteger2 > currentReducedBigInteger )
        {
            if (calculator > 1)
                resultBigInteger._value.insert(resultBigInteger._value.begin(), 0);
        ...
        currentDifference = currentReducedBigInteger.difference(currentBigIntegerMaxDeductible);
        resultBigInteger._value.insert(resultBigInteger._value.begin(), nextNumberInResultBigInteger);
        if (currentDifference > BigInteger<T>(0))
